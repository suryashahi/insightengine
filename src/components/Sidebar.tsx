...
export default function Sidebar({
  ...
  return (
    <aside
      className={`bg-white/5 backdrop-blur-2xl flex flex-col h-screen transition-all duration-300 ease-in-out border-r border-white/10 z-30 shrink-0 ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      {/* Brand Header */}
      ...
      <div className="p-4 border-t border-white/10 shrink-0">
        ...
        >
          ...
          {!collapsed && <span className="text-sm font-semibold tracking-tight">Sign Out</span>}
        </button>
      </div>
    </aside>
  );
